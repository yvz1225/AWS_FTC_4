...
프론트엔드는 React(Vite), 백엔드는 FastAPI(Python), 인프라는 AWS Lambda, EventBridge, SES, DynamoDB를 사용한다.
...
- **Activity_Estimate**: 팀원의 활동 수준을 나타내는 참고용 지표로, Notion 완료 태스크 점수 x 0.5 + GitHub 커밋 수 x 0.5로 산출한다. Notion과 GitHub 중 하나만 선택하거나 둘 다 선택 가능하다
- **Activity_Status**: 팀원의 활동 수준을 3단계로 분류한 배너 상태 (낮음 / 주의 / 정상)
- **Reminder_Service**: EventBridge + Lambda로 1시간마다 실행되어, 내일 마감이면서 미완료(상태가 완료가 아닌) Task를 조회하고, 한국 시간 기준 12:00에 담당자에게 AWS SES를 통해 리마인드 메일을 발송하는 서비스
...
5. WHERE Notion과 GitHub 중 하나만 연동된 경우, THE Tracker_Agent SHALL 연동된 소스의 데이터만으로 Activity_Estimate를 산출한다
6. WHEN Activity_Estimate를 산출하면, THE Dashboard SHALL 각 팀원에 대해 Activity_Status 배너(낮음 / 주의 / 정상)를 퍼센트와 함께 표시한다
7. THE Dashboard SHALL Activity_Estimate가 절대적 평가값이 아닌 참고용 지표임을 명시한다
8. IF GitHub API 또는 Notion API 호출이 실패하면, THEN THE Tracker_Agent SHALL 에러를 로깅하고 실패한 소스를 건너뛴 채 나머지 데이터를 수집한다
...
1. THE Reminder_Service SHALL EventBridge 스케줄에 의해 1시간 주기로 Lambda를 실행한다
...
3. THE System SHALL Tracker_Agent에 대해 EventBridge rate(1 hour) 스케줄 트리거를 구성한다
4. THE System SHALL Reminder_Service에 대해 EventBridge rate(1 hour) 스케줄 트리거를 구성한다
5. THE System SHALL GEMINI_API_KEY, NOTION_API_KEY, NOTION_PARENT_DB_ID, GITHUB_TOKEN 환경 변수를 Lambda 함수에 주입한다
6. THE System SHALL Python 3.12 런타임과 ap-northeast-2 리전을 사용한다
...
4. THE System SHALL 대시보드 UI 화면에서 팀원별 Activity_Status 배너를 퍼센트와 함께 표시한다

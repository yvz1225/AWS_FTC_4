...
GPT 기반 채팅형 명세 생성, Notion 자동 구축, GitHub/Notion 기반 활동 지표 수집, 자동 리마인드 메일 발송, when2meet 스타일 가용시간 조율 UI, 팀원별 활동 대시보드를 제공한다.
...
- **Chat_Agent**: OpenAI GPT API를 활용하여 사용자와 대화형 인터페이스로 프로젝트 명세 정보를 수집하고, 초안을 생성/수정하며, 승인 시 백엔드 검증을 거쳐 Notion 자동 생성을 트리거하는 백엔드 서비스
...
- **Professor_Dashboard**: 교수 전용 읽기 전용 페이지로, 전체 프로젝트 목록과 팀별 기여도 밸런스 상태를 신호등 아이콘(초록/노랑/빨강)으로 한눈에 확인할 수 있다

---

## 3. 요구사항

### 요구사항 1: GPT 기반 채팅형 명세 생성
...
1. WHEN 사용자가 메시지를 전송하면, THE Chat_Agent SHALL OpenAI GPT API를 호출하여 대화 맥락을 유지한 응답을 생성한다
...
7. THE Chat_Agent SHALL 이전 대화 내역(history)을 포함하여 GPT API에 전달함으로써 멀티턴 대화 맥락을 유지한다
8. IF GPT API 호출이 실패하면, THEN THE Chat_Agent SHALL HTTP 500 상태 코드와 에러 메시지를 반환한다
...
5. THE System SHALL OPENAI_API_KEY, NOTION_API_KEY, NOTION_PARENT_DB_ID, GITHUB_TOKEN 환경 변수를 Lambda 함수에 주입한다
...
1. THE System SHALL OPENAI_API_KEY, NOTION_API_KEY, NOTION_PARENT_DB_ID, GITHUB_TOKEN, AWS_REGION을 환경 변수로 관리한다
...
2. THE System SHALL 채팅 UI 화면에서 GPT 기반 명세 생성 대화를 수행할 수 있도록 한다
...
**사용자 스토리:** 교수로서, 담당 수업의 모든 팀 프로젝트 기여도를 한 화면에서 확인하고 싶다. 이를 통해 무임승차 팀원이 있는 팀을 빠르게 파악할 수 있다.

**페이지 구성:**
- 전체 프로젝트 목록을 카드/리스트 형태로 표시
- 각 프로젝트 옆에 신호등 아이콘 (🟢 초록 / 🟡 노랑 / 🔴 빨강)
- 팀원 전원 정상 → 🟢 초록
- 팀원 중 한 명이라도 주의 → 🟡 노랑
- 팀원 중 한 명이라도 기여도 낮음 → 🔴 빨강
- 프로젝트 클릭 시 팀원별 기여도 상세 (퍼센트 + 상태 배너 + 커밋 횟수) 펼침

#### 인수 조건

1. THE System SHALL `/professor` 경로에 교수 전용 읽기 전용 대시보드 페이지를 제공한다
2. WHEN 교수가 페이지에 접근하면, THE Professor_Dashboard SHALL DynamoDB projects 테이블에서 전체 프로젝트 목록을 조회하여 표시한다
3. THE Professor_Dashboard SHALL 각 프로젝트 옆에 팀 기여도 밸런스 상태를 신호등 아이콘(초록/노랑/빨강)으로 표시한다
4. WHEN 팀원 전원의 Activity_Status가 정상이면, THE Professor_Dashboard SHALL 해당 프로젝트에 초록(🟢) 아이콘을 표시한다
5. WHEN 팀원 중 한 명이라도 Activity_Status가 주의이면, THE Professor_Dashboard SHALL 해당 프로젝트에 노랑(🟡) 아이콘을 표시한다
6. WHEN 팀원 중 한 명이라도 Activity_Status가 낮음이면, THE Professor_Dashboard SHALL 해당 프로젝트에 빨강(🔴) 아이콘을 표시한다
7. WHEN 교수가 프로젝트를 클릭하면, THE Professor_Dashboard SHALL 해당 팀의 팀원별 기여도 상세(퍼센트, 상태 배너, 커밋 횟수)를 펼쳐서 표시한다
8. THE Professor_Dashboard SHALL 간단한 접근 제한(비밀번호 또는 추측 불가능한 URL)을 통해 교수만 접근 가능하도록 한다
...
#### 교수님 전용 대시보드 (`/professor`)
- 전체 프로젝트 목록 (카드/리스트)
- 각 프로젝트 옆에 신호등 아이콘 (🟢/🟡/🔴)
  - 전원 정상 → 🟢 초록
  - 한 명이라도 주의 → 🟡 노랑
  - 한 명이라도 기여도 낮음 → 🔴 빨강
- 프로젝트 클릭 시 팀원별 기여도 상세 펼침 (퍼센트 + 상태 배너 + 커밋 횟수)
...
Step 10. 교수님 대시보드 (Professor_Dashboard)
  -> 전체 프로젝트 목록 + 신호등 아이콘 (🟢/🟡/🔴)
  -> 클릭 시 팀원별 기여도 상세 펼침
```
...
- 채팅 기반 명세 생성은 GPT가 담당하되, 최종 저장 전에는 반드시 백엔드 스키마 검증을 거친다.
- 재시도 버튼은 새 Conversation_ID를 발급하여 완전히 새로운 채팅 세션을 시작한다.
- 교수 전용 대시보드는 전체 프로젝트의 기여도 밸런스를 신호등 아이콘으로 한눈에 파악할 수 있도록 제공한다.
